...
class ServerDetailScreen extends StatefulWidget {
  const ServerDetailScreen(
      {super.key, required this.api, required this.serverId});
  final ApiService api;
  final String serverId;
  @override
  State<ServerDetailScreen> createState() => _ServerDetailState();
}
...
class _ServerDetailState extends State<ServerDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      final value = await widget.api.getServer(widget.serverId);
      if (mounted) setState(() => server = value);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: Text(server!.hostname ?? server!.id)),
        body: ListView(
          ...
          children: [
            ...
            if (metric != null) ...[
              ...
              MetricCard(
                  ...
                  subtitle:
                      '${_gb(metric.ram.usedBytes)} / ${_gb(metric.ram.totalBytes)}'),
            ],
            const SizedBox(height: 22),
            Text('Containers', style: Theme.of(context).textTheme.titleLarge),
            for (final container in server!.containers)
              Card(
                  child: ListTile(
